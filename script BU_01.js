...
const polatisArraySrc = [{
    ...
    "initial-connected-port-slot": 0
}, {
    name: "PSU_02_SRC",
     // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "CCU_02_DST",
    "initial-connected-port-slot": 1
}, {
    name: "PSU_03_SRC",
    // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "CCU_03_DST",
    "initial-connected-port-slot": 2
}, {
    name: "PSU_04_SRC",
    // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "CCU_04_DST",
    "initial-connected-port-slot": 3
},]
const polatisArrayDst = [{
    name: "CCU_01_DST",
    // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "PSU_01_DST"
}, {
    name: "CCU_02_DST",
    // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "PSU_02_DST"
}, {
    name: "CCU_03_DST",
    // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "PSU_03_DST"
}, {
    name: "CCU_04_DST",
    // portSideA: each["ingress"][0],
    // portSideB: each["egress"][0],
    "connected-group-name": "PSU_04_DST"
},]
...
const removeOldDstLink = (oldSrcLink = 3) => {
    document.querySelectorAll(".dstSpan").forEach(each => { 
        if (each.value == oldSrcLink) {
            console.log("OLD", oldSrcLink)
        }
    })
}
...
const show_cancel_box = (e) => {
    ...
    console.log("CURRENT", currentConnect)
    document.querySelectorAll(".src_list_cancel_box").forEach(each => {
        each.style.left = "50px"
    })
...
const linkPolatisPorts = async (portSrc, portDst) => {
    ...
    const srcConnectIndex = srcGroupsHold.findIndex(o => o.name === portSrc)
    const removedDstConnectIndex = dstGroupsHold.findIndex(o => o.name === portDst)
    console.log(srcConnectIndex, removedDstConnectIndex)
    
    polatisArraySrc.forEach(each => {
        if(each["initial-connected-port-slot"] == removedDstConnectIndex)
        each["initial-connected-port-slot"] = undefined
    })
    polatisArraySrc[srcConnectIndex]
...
async function PullAndPush() {
            ...
            array.forEach(each => {
                ...
                if(each["group-name"].toLowerCase().includes(srcSearch.toLowerCase())) {
                    ...
                    srcGroupsHold.push({
                        ...
                        portSideB: each["egress"][0],
                        "connected-group-name": each["connected-group"],
                        "initial-connected-port-slot": null
                    })
                }
            })
            srcGroupsHold.forEach(each => {
                // console.log("HERE IS EACH", each)
                each["initial-connected-port-slot"] = dstGroupsHold.findIndex(o => o["connected-group-name"] == each["name"])
    ...
    const appendArrayList = () => {
        polatisArraySrc.forEach(each => {
            ...
            <li class="list_text" onclick="show_cancel_box(this)"} value="${each["initial-connected-port-slot"]}">${each["name"]}</li>
            </ul>
            `
